    ...
    <ion-app>
      ...
      <ion-content class="ion-padding">
        ...
        <ng-template #detail>
          ...
          <ng-template #empty><p>Select a website from the list.</p></ng-template>
        </ng-template>
      </ion-content>
    </ion-app>
  `,
...
export class AppComponent {
  readonly title = "Tutorial Interactive Chip Simulation and Education website";
  readonly screens = ["Websites", "Website Detail"];
  readonly screen = signal("Websites");
  readonly items = signal<Website[]>([]);
  readonly selected = signal<Website | null>(null);
  readonly error = signal('');
  private readonly http = inject(HttpClient);

  constructor() {
    this.http.get<Website[]>(apiUrl('/api/websites')).subscribe({
      next: (items) => this.items.set(items),
      error: () => this.error.set('Could not reach the API.'),
    });
  }

  select(item: Website): void {
